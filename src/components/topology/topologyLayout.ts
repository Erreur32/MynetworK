...
// wifi APs. The link is virtual (not a real cable), and a horizontal row
// crossing through the wired clients next door is visually misleading.
function isVmHostCluster(
  parentId: string,
  nodeById: Map<string, Node>,
): boolean {
  const parent = nodeById.get(parentId);
  const data = parent?.data as TopologyNodeData | undefined;
  return data?.kind === "vm-host";
}
...
// the switch label readable instead of being masked by edge terminations.
const WRAPPED_CLIENT_VGAP_BASE = 50;
const WRAPPED_CLIENT_VGAP_STEP = 8; // px added per client above the threshold
const WRAPPED_CLIENT_VGAP_FROM = 3; // start adding extra space when n > 3
const WRAPPED_CLIENT_VGAP_MAX = 130; // cap to prevent runaway spacing on huge clusters
function wrappedClientVGap(n: number): number {
  if (n <= WRAPPED_CLIENT_VGAP_FROM) return WRAPPED_CLIENT_VGAP_BASE;
  return Math.min(
    WRAPPED_CLIENT_VGAP_MAX,
    WRAPPED_CLIENT_VGAP_BASE +
      (n - WRAPPED_CLIENT_VGAP_FROM) * WRAPPED_CLIENT_VGAP_STEP,
  );
}
...
function computeWrappedDims(
  ...
  for (const [parentId, children] of childrenByParent) {
    ...
    // vm-host (Proxmox / KVM stack): same accordion geometry as wifi, but a
    // distinct placement so the edge renderer can pick the virtual-link
...
function infraReservedHeight(node: Node, dim: WrappedDim | undefined): number {
  ...
  return baseH + wrappedClientVGap(dim.n) + dim.h + bottomPad;
}
...
function equalizeSiblingHeights(
  siblings: string[],
...
function equalizeSiblings(
  ...
  for (const n of infraNodes)
    heights.set(n.id, infraReservedHeight(n, dims.get(n.id)));
...
function placeWifiAccordion(
  ...
  const baseY =
    parent.position.y +
    nodeHeightFor(parent) +
    wrappedClientVGap(children.length);
  const rowH = CLIENT_H + WIFI_CLIENT_VGAP;
  const out: Node[] = [];
...
function placeWrappedChildren(
  ...
  if (dim.placement === "wifi-accordion" || dim.placement === "vm-accordion") {
    ...
    return {
      nodes: placeWifiAccordion(parent, children, clientById),
      nextOrphanX: orphanX,
    };
  }
  if (dim.placement === "wired-row") {
    return {
      nodes: placeWiredRow(parent, children, clientById),
      nextOrphanX: orphanX,
    };
  }
...
function buildHierarchicalLayout(
  ...
  for (const [parentId, children] of childrenByParent) {
    ...
    const placed = placeWrappedChildren(
      parent,
      children,
      clientById,
      dim,
