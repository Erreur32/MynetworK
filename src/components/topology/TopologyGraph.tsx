...
const PORT_LEGEND: Array<{
  key: "portUp" | "portFibre" | "portUplink" | "portDown";
  swatch: string;
}> = [
...
function loadPersistedFilters(): PersistedFilters | null {
  try {
    ...
    const persistedSources = Array.isArray(parsed.sources)
      ? parsed.sources.filter((s) => ALL_SOURCES.includes(s))
      : null;
...
> = {
  ...
  offline: {
    icon: CircleOff,
    activeBg: "bg-rose-500/25 border-rose-400/50 text-rose-100",
  },
  stale: {
    icon: CircleOff,
...
function classifyStatus(node: {
  ...
  if (node.sources.length === 1 && node.sources[0] === "freebox")
    return "stale";
  return "offline";
}
...
const SOURCE_CHIP: Record<
  SourcePlugin,
  { label: string; activeBg: string; dot: string }
> = {
  freebox: {
    label: "Freebox",
    activeBg: "bg-red-500/25 border-red-400/50 text-red-100",
    dot: "bg-red-400",
  },
  unifi: {
    label: "UniFi",
...
> = {
  gateway: {
    icon: RouterIcon,
    activeBg: "bg-amber-500/25 border-amber-400/50 text-amber-100",
  },
  switch: {
    icon: Server,
    activeBg: "bg-emerald-500/25 border-emerald-400/50 text-emerald-100",
  ...
  repeater: {
    icon: Repeat,
    activeBg: "bg-purple-500/25 border-purple-400/50 text-purple-100",
  },
  "vm-host": {
    icon: Layers,
  ...
  client: {
    icon: Smartphone,
    activeBg: "bg-slate-500/25 border-slate-400/50 text-slate-100",
  },
  unknown: {
    icon: HelpCircle,
    activeBg: "bg-slate-600/25 border-slate-500/50 text-slate-200",
  },
};
...
function pickSidePair(dx: number, dy: number): SidePair {
  ...
  if (ratio < 1 / PARALLEL_RATIO)
    return parallelPair(dx >= 0 ? "right" : "left");
...
function resolveEdgeSides(q: EdgeHandleQuery): SidePair {
  ...
  // Virtual links (vm-host → VM) share wifi's accordion-spine geometry: a
  // central bottom source feeding two flanking columns. Treat them as wifi
  // for handle-side resolution.
  const isCurved = q.isWifi || q.isVirtual;
  const portAware = !isCurved && typeof q.portIndex === "number";
  ...
  if (isCurved) target = targetSideFromDx(dx);
  let source = isCurved ? ("bottom" as HandleSide) : pair.source;
...
function pickTargetHandle(
  q: EdgeHandleQuery,
  side: HandleSide,
): { handle: string; side: HandleSide } {
  const isCurved = q.isWifi || q.isVirtual;
  const uplinkAware =
    !isCurved && typeof q.localPortIndex === "number" && q.targetHasUplinkChip;
  if (uplinkAware) return { handle: `pt${q.localPortIndex}`, side: "top" };
  if (!q.sourcePos && q.mode === "editable" && isCurved)
    return { handle: "tl", side: "left" };
...
function pickEdgeHandles(q: EdgeHandleQuery): PickResult {
  ...
  const isCurved = q.isWifi || q.isVirtual;
  const portAware = !isCurved && typeof q.portIndex === "number";
  const source = portAware
    ? `p${q.portIndex}`
    : SOURCE_HANDLE_BY_SIDE[sides.source];
...
function fanIdxForOffset(
  targetXFromCardLeft: number,
  cardWidth: number,
): number {
...
function spreadCentralHandles(
  ...
  for (let i = 0; i < edges.length; i++) {
    const e = edges[i];
    // Wi-Fi AND virtual edges stay on the central `s` handle so all cables
    // from the same parent (AP or vm-host) visually overlap on a single
    // vertical spine going down through the accordion. Spreading them
    // would break the spine effect.
    const d = (e.data ?? {}) as { medium?: EdgeMedium };
    if (d.medium === "wifi" || d.medium === "virtual") continue;
...
const INFRA_OBSTACLE_KINDS = new Set([
  "gateway",
  "switch",
  "ap",
  "repeater",
...
function findClearMidY(
  ...
  const valid = candidates.filter(
    (y) => y >= validMin && y <= validMax && isClear(y),
  );
  if (valid.length === 0) {
    ...
    outOfCorridor.sort(
      (a, b) => Math.abs(a - naturalMidY) - Math.abs(b - naturalMidY),
    );
...
function findClearMidX(
  ...
  const blocking = obstacles.filter(
    (o) => o.y - AVOID_MARGIN < yMax && o.y + o.h + AVOID_MARGIN > yMin,
  );
  const naturalMidX = (sx + tx) / 2;
  if (blocking.length === 0) return naturalMidX;
  const isClear = (x: number): boolean =>
    blocking.every(
      (o) => x < o.x - AVOID_MARGIN || x > o.x + o.w + AVOID_MARGIN,
    );
  if (isClear(naturalMidX)) return naturalMidX;
  const leftX =
    Math.min(...blocking.map((o) => o.x)) - AVOID_MARGIN - AVOID_CLEARANCE_PAD;
  if (isClear(leftX)) return leftX;
  const rightX =
    Math.max(...blocking.map((o) => o.x + o.w)) +
    AVOID_MARGIN +
    AVOID_CLEARANCE_PAD;
...
const AvoidingEdge: React.FC<EdgeProps> = (props) => {
  ...
  let labelX: number;
  let labelY: number;
  if (medium === "wifi" || medium === "virtual") {
    // Wireless link OR virtual (vm-host → VM): curved bezier instead of
    // orthogonal V-H-V. The dashed pattern + curve communicates "this
    // isn't a physical cable" at a glance. Obstacle avoidance is skipped
    // here on purpose — both wifi and virtual links use the flanking
    // accordion layout, which already positions cards to avoid overlaps.
...
function pickChipClass(
  disabled: boolean,
  active: boolean,
  activeBg: string,
): string {
  if (disabled)
    return "opacity-40 cursor-not-allowed border-slate-800 text-slate-500";
...
async function exportFlowElement(
  flowEl: HTMLElement,
  format: "png" | "svg" | "pdf",
  stamp: string,
): Promise<void> {
...
export const TopologyGraph: React.FC<TopologyGraphProps> = ({
  ...
  const [snapEnabled, setSnapEnabled] = useState<boolean>(() => {
    try {
      return globalThis.localStorage?.getItem(SNAP_STORAGE_KEY) === "1";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    let cancelled = false;
    api
      .get<Record<string, { x: number; y: number }>>("/api/topology/positions")
      .then((resp) => {
        if (cancelled) return;
      ...
      .catch(() => {
        /* swallow — graph still renders with dagre layout */
      });
    return () => {
      cancelled = true;
    };
  ...
  const handleNodeDragStop = useCallback((_e: MouseEvent | TouchEvent, node: Node) => {
    ...
    api.post("/api/topology/positions", { nodeId: node.id, x, y }).catch(() => {
      /* best-effort */
    });
  ...
  const sourceCounts = useMemo(() => {
    const counts: Record<SourcePlugin, number> = {
      freebox: 0,
      unifi: 0,
      "scan-reseau": 0,
    };
    for (const n of graph.nodes)
      for (const s of n.sources) counts[s] = (counts[s] ?? 0) + 1;
    return counts;
  }, [graph]);
  ...
  const layouted = useMemo(() => {
    ...
    const parentConnByClient = new Map<
      string,
      {
    ...
    const rfNodes: Node[] = filteredGraph.nodes.map((n) => ({
      ...
      data: {
        ...
        vmCount:
          typeof n.metadata?.vmCount === "number"
            ? n.metadata.vmCount
            : undefined,
        vmActiveCount:
          typeof n.metadata?.vmActiveCount === "number"
            ? n.metadata.vmActiveCount
            : undefined,
        vmInactiveCount:
          typeof n.metadata?.vmInactiveCount === "number"
            ? n.metadata.vmInactiveCount
            : undefined,
        hypervisor:
        ...
        modelDisplay:
          typeof n.metadata?.modelDisplay === "string"
            ...
            : undefined,
        portsFromSnapshot:
          n.metadata?.portsFromSnapshot === true ? true : undefined,
        hasDedicatedUplink:
          typeof n.metadata?.hasDedicatedUplink === "boolean"
            ? n.metadata.hasDedicatedUplink
            : undefined,
    ...
    const rfEdges: Edge[] = filteredGraph.edges.map((e) => {
      ...
      const chipRendered = targetNode
        ? shouldRenderUplinkChip(
            targetNode.kind,
            targetNode.metadata?.hasDedicatedUplink,
          )
        : false;
      const targetHasUplinkChip =
        chipRendered &&
  ...
  const edges = useMemo<Edge[]>(() => {
    const initial: Edge[] = layouted.edges.map((e) => {
      ...
      const isVirtual = d.medium === "virtual";
      ...
      const baseStrokeWidth =
        typeof baseStyle.strokeWidth === "number" ? baseStyle.strokeWidth : 1.6;
      const stroke =
      ...
      const edgeObstacles = obstaclesAll.filter(
        (o) => o.id !== e.source && o.id !== e.target,
      );
      return {
        ...e,
        type: "avoiding",
  ...
  const selectedNode = useMemo(
    () =>
      selectedId
        ? (filteredGraph.nodes.find((n) => n.id === selectedId) ?? null)
        : null,
  ...
  const selectedEdges = useMemo(() => {
    if (!selectedId) return [];
    const list = filteredGraph.edges.filter(
  ...
  const selectedEdgePoe = useMemo<boolean | null>(() => {
    if (!selectedEdge || typeof selectedEdge.portIndex !== "number")
      return null;
    const src = filteredGraph.nodes.find((n) => n.id === selectedEdge.source);
    const port = src?.metadata?.ports?.find(
      (p) => p.idx === selectedEdge.portIndex,
    );
  ...
  return (
    <div
      className="relative rounded-xl border border-slate-700 overflow-hidden bg-slate-950"
      style={{ height }}
    >
              ...
              {ALL_SOURCES.map((src) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      disabled ||
                      setSourceFilter((prev) => toggleSet(prev, src))
                    }
                    disabled={disabled}
              ...
              {ALL_STATUS.map((s) => {
                ...
                return (
                  <button
                    ...
                    title={
                      s === "stale" ? t("topology.status.staleHint") : undefined
                    }
                  >
                    <Icon size={11} />
      ...
      <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 flex items-center gap-0.5 p-1 rounded-lg bg-slate-900/90 border border-slate-700 shadow-lg">
        ...
        {editableMode && (
          <button
            ...
            title={
              dragMode ? t("topology.editLayoutOn") : t("topology.editLayout")
            }
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="hidden sm:inline">
              {exporting
                ? t("topology.export.exporting")
                : t("topology.export.button")}
            </span>
          </button>
          {exportMenuOpen && (
            <div className="absolute top-full mt-1 right-0 z-20 min-w-[160px] rounded-lg bg-slate-900/95 border border-slate-700 shadow-lg overflow-hidden">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-xs text-slate-200 hover:bg-slate-800 transition-colors"
              >
                <ImageIcon size={13} className="text-sky-300" />
                <span>PNG</span>
                <span className="ml-auto text-[10px] text-slate-500">
                  {t("topology.export.pngHint")}
                </span>
              </button>
              <button
                onClick={() => exportFile("pdf")}
                className="w-full flex items-center gap-2 px-3 py-2 text-xs text-slate-200 hover:bg-slate-800 transition-colors border-t border-slate-800"
              >
      ...
      {selectedNode &&
        (() => {
          const kindAccent = NODE_KIND_LEGEND.find(
            (k) => k.id === selectedNode.kind,
          );
              ...
              <div className="flex items-start justify-between gap-2 p-4 border-b border-slate-700">
                <div className="min-w-0">
                  <div className="text-xs uppercase tracking-wide text-slate-400">
                    {t(`topology.kind.${selectedNode.kind}`)}
                  </div>
              ...
              <div className="p-4 space-y-3 text-sm">
                ...
                {selectedNode.vendor && (
                  <DetailRow
                    ...
                    label={t("topology.detail.vendor")}
                ...
                {selectedNode.metadata?.ssid &&
                  (() => {
                    const band = selectedNode.metadata.band
                      ? ` (${selectedNode.metadata.band})`
                      : "";
                    const ssidValue = `${selectedNode.metadata.ssid}${band}`;
                    return (
                      <DetailRow
                        icon={<Wifi size={14} />}
                        label={t("topology.detail.ssid")}
                        value={ssidValue}
                      />
                    );
                  })()}
                {(selectedNode.kind === "switch" ||
                  ...
                  selectedNode.metadata.ports.length > 0 &&
                  (() => {
                    ...
                    const totalSpeed = ports
                      .filter((p) => p.up)
                      .reduce((s, p) => s + (p.speed ?? 0), 0);
                    const speedLabel =
                      totalSpeed >= 1000
                        ? `${(totalSpeed / 1000).toFixed(1)} Gbps`
                        : `${totalSpeed} Mbps`;
                    return (
                        ...
                        <div className="grid grid-cols-3 gap-2 text-xs">
                          <div className="bg-slate-700/40 rounded px-2 py-1.5">
                            <div className="text-[10px] uppercase tracking-wide text-slate-500">
                              {t("topology.detail.portsUp")}
                            </div>
                            <div className="font-mono text-slate-100">
                              {upCount} / {ports.length}
                            </div>
                          </div>
                          <div className="bg-slate-700/40 rounded px-2 py-1.5">
                            <div className="text-[10px] uppercase tracking-wide text-slate-500">
                              {t("topology.detail.portsPoe")}
                            </div>
                ...
                {(selectedNode.kind === "ap" ||
                  selectedNode.kind === "repeater") && (
                  <WifiSummaryBlock
      ...
      {!selectedNode && selectedEdge && (
        <EdgeInfoPanel
          edge={selectedEdge}
          sourceLabel={
            nodeLabelById.get(selectedEdge.source) ??
            selectedEdge.source.replace(/^mac:/, "")
          }
          targetLabel={
            nodeLabelById.get(selectedEdge.target) ??
            selectedEdge.target.replace(/^mac:/, "")
          }
      ...
      {dragMode && (
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1 p-1.5 rounded-lg bg-slate-900/95 border border-amber-400/40 shadow-lg shadow-amber-500/10">
          <span className="px-2 text-[10px] uppercase tracking-wide text-amber-200 font-semibold">
            {selectedNode
              ? `${t("topology.editToolbar.move")}: ${selectedNode.label}`
              : t("topology.editToolbar.selectNode")}
          </span>
          ...
          <button
            ...
            title={
              snapEnabled
                ? t("topology.editToolbar.snapOn")
                : t("topology.editToolbar.snapOff")
            }
          ...
          >
            ...
            <span
              className={`font-mono text-[9px] ${snapEnabled ? "text-sky-200" : "text-slate-500"}`}
            >
              {snapEnabled
                ? `${SNAP_GRID[0]}px`
                : t("topology.editToolbar.snapOff")}
          ...
          <div className="px-3 pb-2 pt-1 border-t border-slate-800 space-y-2 text-[11px] text-slate-300">
            <div>
              <div className="text-[9px] uppercase tracking-widest text-slate-500 mb-1">
                {t("topology.legend.links")}
              </div>
              <div className="flex flex-wrap gap-x-3 gap-y-1">
                <span className="flex items-center gap-1.5">
                  <span
                    className="w-3 h-0.5 inline-block"
                    style={{ backgroundColor: EDGE_COLOR.ethernet }}
                  />{" "}
                  {t("topology.legend.ethernet")}
                </span>
                <span className="flex items-center gap-1.5">
                  <span
                    className="w-3 inline-block"
                    style={{ borderTop: `2px dashed ${EDGE_COLOR.wifi}` }}
                  />{" "}
                  {t("topology.legend.wifi")}
                </span>
                <span className="flex items-center gap-1.5">
                  <span
                    className="w-3 h-0.5 inline-block"
                    style={{ backgroundColor: EDGE_COLOR.uplink }}
                  />{" "}
                  {t("topology.legend.uplink")}
                </span>
                <span className="flex items-center gap-1.5">
                  <span
                    className="w-3 inline-block"
                    style={{ borderTop: `2px dashed ${EDGE_COLOR.virtual}` }}
                  />{" "}
                  {t("topology.legend.virtual")}
                </span>
              </div>
            </div>
            <div>
              <div className="text-[9px] uppercase tracking-widest text-slate-500 mb-1">
                {t("topology.legend.nodes")}
              </div>
              <div className="flex flex-wrap gap-x-3 gap-y-1">
                {NODE_KIND_LEGEND.map((k) => (
                  <span key={k.id} className="flex items-center gap-1.5">
                    <span className={`w-2 h-2 rounded-sm ${k.bar}`} />{" "}
                    {t(`topology.kind.${k.id}`)}
                  </span>
                ))}
              </div>
            </div>
            <div>
              <div className="text-[9px] uppercase tracking-widest text-slate-500 mb-1">
                {t("topology.legend.ports")}
              </div>
              <div className="flex flex-wrap gap-x-3 gap-y-1">
                {PORT_LEGEND.map((p) => (
                  <span key={p.key} className="flex items-center gap-1.5">
                    <span
                      className={`w-3.5 h-3 rounded-sm border ${p.swatch}`}
                    />
...
interface SsidAggregate {
  count: number;
  bands: Set<string>;
}

function aggregateBySsid(
  wifiEdges: TopologyEdgeIn[],
): Map<string, SsidAggregate> {
...
const WifiSummaryBlock: React.FC<WifiSummaryBlockProps> = ({
  selectedNodeId,
  edges,
  t,
}) => {
  const wifiOut = edges.filter(
    (e) => e.medium === "wifi" && e.source === selectedNodeId,
  );
  ...
  return (
    <div className="pt-2 border-t border-slate-700 space-y-1.5">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="bg-slate-700/40 rounded px-2 py-1.5">
          <div className="text-[10px] uppercase tracking-wide text-slate-500">
            {t("topology.detail.wifiClients")}
          </div>
          <div className="font-mono text-sky-200">{wifiOut.length}</div>
        </div>
        <div className="bg-slate-700/40 rounded px-2 py-1.5">
          <div className="text-[10px] uppercase tracking-wide text-slate-500">
            {t("topology.detail.wifiTotal")}
          </div>
        ...
        <div className="space-y-1">
          {Array.from(bySsid.entries()).map(([ssid, info]) => (
            <SsidRow
              ...
              ssid={ssid}
              count={info.count}
              bandsText={formatSsidBands(info.bands)}
            />
          ))}
        </div>
      )}
...
const ScanDetailsBlock: React.FC<ScanDetailsBlockProps> = ({ ip, t }) => {
  ...
  useEffect(() => {
    ...
    setPorts(undefined);
    api
      .get<NetworkScanLookup>(`/api/network-scan/${encodeURIComponent(ip)}`)
      .then((resp) => {
      ...
      .catch(() => {
        /* device not in scan-reseau, ignore */
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="pt-2 border-t border-slate-700 space-y-2">
      {hasLatency && (
        <DetailRow
          icon={<Gauge size={14} />}
          label={t("topology.detail.latency")}
          value={`${latency} ms`}
          mono
        />
      )}
...
const SsidRow: React.FC<{ ssid: string; count: number; bandsText: string }> = ({
  ssid,
  count,
  bandsText,
}) => (
  <div className="flex items-center justify-between gap-2 text-[11px] bg-slate-700/30 rounded px-2 py-1">
    <span className="truncate text-slate-200" title={ssid}>
      {ssid}
    </span>
...
function buildWifiMetaLine(
  edge: TopologyEdgeIn,
  speedTxt: string | null,
): string {
...
const EdgeRow: React.FC<EdgeRowProps> = ({
  edge,
  selectedNodeId,
  nodeLabelById,
}) => {
  ...
  const showWifiMeta =
    isWifi &&
    (edge.ssid || edge.band || speedTxt || typeof edge.signal === "number");
  return (
    <div
      className={`flex items-start gap-2 text-xs px-1.5 py-1 rounded ${isWifi ? "bg-sky-500/5" : ""}`}
    >
      {isWifi ? (
        <Wifi
          size={12}
          className={`flex-none mt-0.5 ${EDGE_ICON_COLOR.wifi}`}
        />
      ) : (
        <Cable
          size={12}
          className={`flex-none mt-0.5 ${EDGE_ICON_COLOR[edge.medium]}`}
        />
      )}
      <span
        className={`flex-none font-mono text-[10px] font-bold px-1.5 py-0.5 rounded border ${PORT_BADGE_CLASS[edge.medium]} ${hasPort ? "" : "opacity-50"}`}
      >
        {portBadge}
      </span>
      <div className="min-w-0 flex-1">
        <div className="text-slate-200 truncate" title={otherLabel}>
          {otherLabel}
        </div>
        {showWifiMeta && (
          <div className="grid grid-cols-[1fr_auto] gap-x-2 mt-0.5 text-[10px] text-slate-400 leading-tight">
            <span className="truncate" title={edge.ssid ?? ""}>
              {wifiSsidLine}
            </span>
      ...
      {!isWifi && speedTxt && (
        <span className="ml-auto text-slate-400 font-mono whitespace-nowrap">
          {speedTxt}
        </span>
      )}
    </div>
...
const DetailRow: React.FC<DetailRowProps> = ({ icon, label, value, mono }) => (
  <div className="flex items-start gap-2">
    ...
    <div className="min-w-0 flex-1">
      ...
      <div
        className={`text-slate-200 truncate ${mono ? "font-mono text-xs" : ""}`}
      >
        {value}
      </div>
    </div>
    ...
    <div className="absolute top-3 right-3 w-96 lg:w-[28rem] max-h-[calc(100%-1.5rem)] overflow-y-auto rounded-xl border-2 border-slate-600 bg-slate-800 shadow-2xl">
      ...
      <div className="p-4 space-y-3 text-sm">
        {speed && (
          <DetailRow
            icon={<Gauge size={14} />}
            label={t("topology.edgeInfo.speed")}
            value={speed}
            mono
          />
        )}
        ...
        {poeActive !== null && (
          <DetailRow
            icon={
              <Zap
                size={14}
                className={poeActive ? "text-amber-300" : "text-slate-500"}
              />
            }
            label={t("topology.edgeInfo.poe")}
            value={
              poeActive
                ? t("topology.edgeInfo.poeActive")
                : t("topology.edgeInfo.poeInactive")
            }
          />
        )}
        <DetailRow
          icon={<Tag size={14} />}
          label={t("topology.edgeInfo.source_plugin")}
          value={edge.source_plugin}
        />
      </div>
    </div>
  );
