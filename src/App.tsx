...
const UnifiedDashboardPage = lazy(() =>
  import("./pages/UnifiedDashboardPage").then((m) => ({
    default: m.UnifiedDashboardPage,
  })),
);
const UniFiPage = lazy(() =>
  import("./pages/UniFiPage").then((m) => ({ default: m.UniFiPage })),
);
const SearchPage = lazy(() =>
  import("./pages/SearchPage").then((m) => ({ default: m.SearchPage })),
);
const NetworkScanPage = lazy(() =>
  import("./pages/NetworkScanPage").then((m) => ({
    default: m.NetworkScanPage,
  })),
);
...
import {
  ...
  AnimationParametersContext,
} from "./hooks/useAnimationParameters";
    ...
    <div className="mb-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
      ...
      >
        <span className="text-sm font-medium text-amber-400">
          ...
          {entries.length > 1 &&
            ` (${entries.map((e) => `v${e.version}`).join(", ")})`}
        </span>
        {expanded ? (
          <ChevronUp size={18} className="text-amber-400 flex-shrink-0" />
        ) : (
          <ChevronDown size={18} className="text-amber-400 flex-shrink-0" />
        )}
      </button>
      {expanded && (
...
const App: React.FC = () => {
  ...
  const {
    isLoggedIn: isFreeboxLoggedIn,
    isLoading: freeboxAuthLoading,
    checkAuth: checkFreeboxAuth,
    logout: freeboxLogout,
  } = useAuthStore();

  // Plugin store for multi-source dashboard
  const { plugins, pluginStats, fetchPlugins, fetchAllStats } =
    usePluginStore();
  ...
  const { capabilities, supportsVm, hasLimitedVmSupport, getMaxVms } =
  ...
  const [wifiModalTab, setWifiModalTab] = useState<
    "filter" | "planning" | "wps"
  >("filter");
  ...
  const [filesPageInitialTab, setFilesPageInitialTab] = useState<
    "files" | "downloads" | "shares"
  >("files");
  const [filesPageInitialDownloadId, setFilesPageInitialDownloadId] = useState<
    string | undefined
  >(undefined);

  // Filters for history
  const [historyFilter, setHistoryFilter] = useState<
    "all" | "connection" | "calls" | "notifications"
  >("all");
  const [historyPeriod, setHistoryPeriod] = useState<"30d" | "7d" | "24h">(
    "30d",
  );
      ...
      import("./utils/themeManager").then(({ initTheme }) => {
        initTheme().catch((err) => {
          ...
          if (import.meta.env.DEV) {
            console.debug(
              "[Theme] Failed to load custom colors after auth:",
              err,
            );
          }
        });
  ...
  const { isConnected: wsConnected } = useConnectionWebSocket({
    enabled: isFreeboxLoggedIn && !isDockerDev,
  });
  ...
  useEffect(() => {
    if (isFreeboxLoggedIn && isUserAuthenticated) {
      ...
      if (import.meta.env.DEV) {
        console.log(
          "[App] Freebox logged in, fetching initial connection status...",
        );
      }
      fetchConnectionStatus();
  ...
  const hasDisk =
    systemInfo?.disk_status === "active" || systemInfo?.user_main_storage;
  ...
  const filteredHistoryLogs = historyLogs.filter((log) => {
    // Filter by type
    if (historyFilter === "connection" && !log.id.startsWith("conn-"))
      return false;
    if (historyFilter === "calls" && !log.id.startsWith("call-")) return false;
    if (historyFilter === "notifications" && !log.id.startsWith("notif-"))
      return false;
  ...
  const handleVmToggle = async (id: string, start: boolean) => {
    try {
      ...
      toast.error(
        start ? "Échec du démarrage de la VM" : "Échec de l'arrêt de la VM",
      );
    }
  };
    ...
    <AnimationParametersContext.Provider value={animationParameters}>
      <div className="relative min-h-screen">
        ...
        <div className="relative z-0 min-h-screen pb-20 bg-theme-primary/95 text-theme-primary font-sans selection:bg-accent-primary/30">
          <Suspense fallback={<PageLoader t={t} />}>{pageContent}</Suspense>
          <Footer
  ...
  if (currentPage === "settings") {
    ...
    const adminTab = sessionStorage.getItem("adminTab") as
      | "general"
      | "users"
      | "plugins"
      | "logs"
      ...
      <>
        ...
        <main className="p-4 md:p-6 max-w-[1920px] mx-auto">
          <Suspense fallback={<PageLoader t={t} />}>
            <SearchPage onBack={() => setCurrentPage("dashboard")} />
          </Suspense>
        </main>
      </>,
    );
  ...
  const shouldShowFreeboxLogin =
    isFreeboxPluginEnabled &&
    !isFreeboxLoggedIn &&
    !freeboxAuthLoading &&
    isUserAuthenticated;

  // Render Freebox Dashboard (if Freebox plugin is active and connected)
  const isFreeboxPluginActive =
    isFreeboxPluginEnabled && freeboxPlugin?.connectionStatus;
  ...
  if (currentPage === "freebox" && isFreeboxPluginActive) {
    return (
      <div className="min-h-screen pb-20 bg-theme-primary text-theme-primary font-sans selection:bg-accent-primary/30">
        ...
        <main className="p-4 md:p-6 max-w-[1920px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            ...
            <div className="flex flex-col gap-6">
              ...
              {isFreeboxLoggedIn && <NetworkSummaryFreeboxWidget />}
                  ...
                  <div className="flex flex-col gap-4">
                    {import.meta.env.DEV && (
                      <div className="text-xs text-gray-500 mb-2">
                        Debug: history length={networkHistory?.length || 0},
                        status={connectionStatus ? "OK" : "null"}
                      </div>
                    )}
                    <BarChart
                      data={networkHistory || []}
                ...
                <Card
                  ...
                  actions={
                    <button className="text-xs bg-[#1a1a1a] border border-gray-700 px-2 py-1 rounded flex items-center gap-1 text-gray-400">
                      <Calendar size={12} />{" "}
                      <span>{t("freebox.page.period30d")}</span>
                    </button>
                  }
                >
                  {systemInfo ? (
                    <UptimeGrid uptimeSeconds={systemInfo.uptime_val} />
                  ) : (
                ...
                actions={
                  <div className="flex flex-wrap gap-1 sm:gap-2">
                    ...
                    <ActionButton
                      ...
                      icon={Calendar}
                      onClick={() => {
                        setWifiModalTab("planning");
                        setIsWifiModalOpen(true);
                      }}
                    />
                    <ActionButton
                      label={t("dashboard.wps")}
                      icon={WifiIcon}
                      onClick={() => {
                        setWifiModalTab("wps");
                        setIsWifiModalOpen(true);
                ...
                actions={
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        setDeviceFilter(
                          deviceFilter === "active" ? "all" : "active",
                        )
                      }
                    ...
                    <button
                      onClick={() =>
                        setDeviceFilter(
                          deviceFilter === "inactive" ? "all" : "inactive",
                        )
                      }
                      className={`text-xs px-2 py-1 rounded border transition-colors flex items-center gap-1 ${
                ...
                actions={
                  <div className="flex gap-2">
                    ...
                    >
                      <Clock size={12} />
                      {historyPeriod === "30d"
                        ? t("freebox.page.period30d")
                        : historyPeriod === "7d"
                          ? t("freebox.page.period7d")
                          : t("freebox.page.period24h")}
                    </button>
                  </div>
                }
                className={
                  filteredHistoryLogs.length === 0 && historyLogs.length === 0
                    ? ""
                    : "h-full"
                }
              >
        ...
        {shouldShowFreeboxLogin && <LoginModal isOpen={true} />}
      </div>
    );
  }

  // Default return - fallback to dashboard if currentPage is invalid.
  // Plugin-gated pages (freebox / unifi) hit this path briefly while the
  // plugin connection status is still loading — that's a noisy false alarm,
  // not a real "invalid page", so silence the warning for those.
  const PLUGIN_GATED_PAGES = ["freebox", "unifi"];
  if (!PLUGIN_GATED_PAGES.includes(currentPage)) {
    console.warn(
      "[App] Invalid currentPage, falling back to dashboard:",
      currentPage,
    );
  }
  return renderPageWithFooter(
