...
class ModelDetectionService {
  ...
  private async _performDetection(): Promise<FreeboxCapabilities> {
    ...
    if (mockModel && MOCK_MODEL_NAMES[mockModel]) {
      ...
      logger.info('ModelDetection', `MOCK MODE: Simulating ${mock.name} (MOCK)`);
      ...
      logger.info('ModelDetection', 'Mock Capabilities:', {
    ...
    try {
      logger.info('ModelDetection', 'Starting model detection...');

      const apiVersion = await freeboxApi.getApiVersion();

      if (!apiVersion.success || !apiVersion.result) {
        logger.warn('ModelDetection', 'Failed to get API version, using defaults');
      ...
      logger.info('ModelDetection', 'API response:', {
      ...
      logger.info('ModelDetection', `Using model name: "${modelName}"`);
      ...
      if (hasRealStorage && !this.capabilities.hasInternalStorage) {
        logger.info('ModelDetection', 'Detected internal/sata/nvme disk - overriding hasInternalStorage to true');
        this.capabilities.hasInternalStorage = true;
      }

      this.lastDetectionTime = Date.now();

      logger.info('ModelDetection', `Detected: ${modelName} -> ${model}`);
      logger.info('ModelDetection', 'Capabilities:', {
        model: this.capabilities.model,
        wifi6ghz: this.capabilities.wifi6ghz,
        vmSupport: this.capabilities.vmSupport,
        hasInternalStorage: this.capabilities.hasInternalStorage
      ...
      logger.error('ModelDetection', 'Detection failed:', error);
      return this.buildDefaultCapabilities();
    }
  }
  ...
  private async checkActualStorage(): Promise<boolean> {
    try {
      ...
      if (hasInternalDisk) {
        logger.info('ModelDetection', 'Found internal/sata/nvme disk(s):', disks.filter(d => internalTypes.includes(d.type || '')).map(d => d.type));
      }

      return hasInternalDisk;
    } catch (error) {
      logger.debug('ModelDetection', 'Could not check storage disks:', error);
      return false;
    }
  }
  ...
  clearCache(): void {
    ...
    logger.debug('ModelDetection', 'Cache cleared');
  }
