...
class FreeboxFirmwareCheckService {
  ...
  private parseBlogContent(html: string): { server: FirmwareEntry | null; player: FirmwareEntry | null } {
    ...
    for (const block of blocks) {
      ...
      const playerMatch = blockTrimmed.match(/Mise\s{1,5}[àa]\s{1,5}jour\s{1,5}du\s{1,5}Freebox\s{1,5}Player\s{1,5}([^\d]{1,50}?)\s{1,5}(\d{1,5}(?:\.\d{1,5}){1,3})/i); // NOSONAR S5852 bounded quantifiers
      if (playerMatch) {
